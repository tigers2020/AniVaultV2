...
def test_make_tmdb_search_execute_handles_cancel_blank_and_fallback() -> None:
    ...
    provider = SimpleNamespace(
        search_series=lambda query, year=None: provider_calls.append((query, year)) or (
            [] if query == "show extra" else ["hit"]
        )
    )
    execute = make_tmdb_search_execute(provider)

    cancelled = execute(SimpleNamespace(query="show", year=2024), None, SimpleNamespace(is_set=lambda: True))
    blank = execute(SimpleNamespace(query="   ", year=None), None, SimpleNamespace(is_set=lambda: False))
    found = execute(SimpleNamespace(query="show extra", year=2024), None, SimpleNamespace(is_set=lambda: False))
...
def test_container_factory_helpers_wire_dependencies(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(container_module, "SqliteLibraryIndexRepository", lambda conn, lock: "library-index")
    monkeypatch.setattr(container_module, "SqliteParseCacheRepository", lambda conn, lock: "parse-cache")
    monkeypatch.setattr(container_module, "SqliteTitleGroupRepository", lambda conn, lock: "title-groups")
    monkeypatch.setattr(container_module, "SqliteTitleMatchRepository", lambda conn, lock: "title-match")
    ...
    monkeypatch.setattr(container_module, "load_all", lambda: {"parse_tmdb": {"ignore_tokens": "x264"}})
    monkeypatch.setattr(container_module, "AnitopyTitleParser", lambda ignore_tokens="": ("parser", ignore_tokens))
    monkeypatch.setattr(container_module, "make_parse_execute", lambda parser, **kwargs: ("execute", parser, kwargs))
    parse_execute = _create_parse_execute(SimpleNamespace(library_index="library", parse_cache="cache"))
    assert parse_execute[0] == "execute"

    monkeypatch.setattr(container_module, "TmdbApiClient", lambda api_key, language="": ("client", api_key, language))
    monkeypatch.setattr(container_module, "TmdbMetadataProvider", lambda client: ("provider", client))
    monkeypatch.setattr(
        container_module,
        "CachingMetadataProvider",
        lambda inner, title_match, language="": ("cached", inner, title_match, language),
    )
    metadata = _create_metadata_provider("key", SimpleNamespace(title_match="tmdb"))
    assert metadata[0] == "cached"

    monkeypatch.setattr(container_module, "SettingsPresenter", lambda: "presenter")
    monkeypatch.setattr(container_module, "SettingsPage", lambda presenter=None: ("settings-page", presenter))
...
def test_create_organizer_page_handles_api_key_and_scan_extensions(monkeypatch) -> None:
    ...
    monkeypatch.setattr(container_module, "make_scan_execute", lambda *args, **kwargs: ("scan", args, kwargs))
    monkeypatch.setattr(container_module, "_create_parse_execute", lambda repos: "parse")
    monkeypatch.setattr(container_module, "make_cached_tmdb_hydrate_execute", lambda **kwargs: "hydrate")
    ...
    monkeypatch.setattr(container_module, "OrganizerPresenterPorts", lambda **kwargs: ("ports", kwargs))
    monkeypatch.setattr(container_module, "OrganizerPresenter", lambda **kwargs: ("presenter", kwargs))
    monkeypatch.setattr(container_module, "OrganizerPage", lambda **kwargs: ("page", kwargs))
    monkeypatch.setattr(container_module, "TmdbPosterAssetSync", lambda *args: SimpleNamespace(sync_from_match_result="syncer"))
    ...
    monkeypatch.setattr(container_module, "_create_metadata_provider", lambda api_key, repos: "metadata")

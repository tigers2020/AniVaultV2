...
def test_execute_worker_helpers_handle_cancel_and_hydrate() -> None:
    ...
    assert module._execute_title_groups_sync_worker(7, None, token, sync_fn=sync_calls.append) is None
    assert sync_calls == [7]
...
def test_scan_path_is_usable_directory_covers_success_and_errors(monkeypatch) -> None:
    ...
    monkeypatch.setattr(coord, "_warn_scan_path", lambda title, message: warnings.append((title, message)))
...
def test_on_scan_thread_finished_only_when_handoff_not_done() -> None:
    presenter = SimpleNamespace(_scan_progress_handoff_done=False, _finish_worker_session=MagicMock())
    coord = _coord(presenter)
    dialog = MagicMock()
...
def test_on_scan_result_sets_rows_or_starts_parse(monkeypatch) -> None:
    ...
    monkeypatch.setattr(coord, "_start_parse_worker", lambda rows, merged, index_root_id: started.append((rows, merged, index_root_id)))
...
def test_scan_result_to_rows_maps_paths_and_resolutions() -> None:
    ...
    rows = coord._scan_result_to_rows(ScanResult(paths=["a.mkv", "b.mkv"], resolutions=["FHD"], index_root_id=1))
...
def test_on_parse_result_ignores_stale_generation_and_merges_rows(monkeypatch) -> None:
    ...
    presenter = SimpleNamespace(_model=model, _parse_index_root_id=5, _sync_title_groups_execute=None)
    coord = _coord(presenter)
    coord._parse_apply_generation = 2  # type: ignore[attr-defined]
    monkeypatch.setattr(coord, "_apply_parse_result_rows_after_optional_hydrate", lambda *args, **kwargs: applied.append((args, kwargs)))
    ...
    coord._on_parse_result(
        ParseResult(parsed=[ParsedInfo(title="Show", parse_group="Group", year="2024", season="1", episode="2", resolution="UHD")]),
        2,
    )
...
def test_apply_parse_result_rows_after_optional_hydrate_tracks_pending_job(monkeypatch) -> None:
    applied: list[object] = []
    presenter = SimpleNamespace(_cached_tmdb_hydrate_execute=object())
    coord = _coord(presenter)
    monkeypatch.setattr(coord, "_apply_parse_result_groups_chunked", lambda *args, **kwargs: applied.append((args, kwargs)))
    monkeypatch.setattr(module, "pipeline_row_to_match_file", lambda row: SimpleNamespace(original_file=row.original_file))
    monkeypatch.setattr(module, "group_pipeline_rows", lambda rows: ["group"])
...
def test_run_pending_cached_tmdb_hydrate_starts_worker(monkeypatch) -> None:
    ...
    monkeypatch.setattr(module, "UseCaseWorker", _Worker)
    monkeypatch.setattr(module, "run_worker", lambda worker: thread)
    presenter = SimpleNamespace(_cached_tmdb_hydrate_execute=lambda dto: MatchResult(files=()), register_worker_thread=MagicMock())
    coord = _coord(presenter)
    coord._parse_apply_generation = 2  # type: ignore[attr-defined]
    coord._pending_cached_hydrate[2] = (MagicMock(), MatchInput())  # type: ignore[attr-defined]

    coord._run_pending_cached_tmdb_hydrate(2)
...
def test_on_cached_tmdb_hydrate_result_applies_grouped_rows_when_current(monkeypatch) -> None:
    applied: list[object] = []
    presenter = SimpleNamespace(_match_file_to_pipeline_row=lambda file: _row(file.original_file))
    coord = _coord(presenter)
    coord._parse_apply_generation = 1  # type: ignore[attr-defined]
    monkeypatch.setattr(module, "group_pipeline_rows", lambda rows: ["group"])
    monkeypatch.setattr(coord, "_apply_parse_result_groups_chunked", lambda *args, **kwargs: applied.append((args, kwargs)))

    coord._on_cached_tmdb_hydrate_result(MatchResult(files=(SimpleNamespace(original_file="a.mkv"),)), MagicMock(), 1)

    assert applied


def test_after_parse_result_groups_applied_syncs_panel_and_optionally_runs_workers(monkeypatch) -> None:
    panel = SimpleNamespace(sync_views_from_model=MagicMock())
    presenter = SimpleNamespace(_pipeline_panel=panel, _notify_dry_run=MagicMock())
    coord = _coord(presenter)
    monkeypatch.setattr(coord, "_run_pending_cached_tmdb_hydrate", MagicMock())
    monkeypatch.setattr(coord, "_run_title_groups_sync_worker", MagicMock())

    coord._after_parse_result_groups_applied(session_gen=4, root_for_sync=8, sync_fn=lambda root: None)
...
def test_schedule_parse_result_chunk_work_processes_all_chunks(monkeypatch) -> None:
    ...
    monkeypatch.setattr(coord, "_after_parse_result_groups_applied", lambda **kwargs: finished.append(kwargs))
...
def test_apply_parse_result_groups_chunked_clears_model_and_schedules(monkeypatch) -> None:
    ...
    monkeypatch.setattr(coord, "_schedule_parse_result_chunk_work", lambda *args, **kwargs: scheduled.append((args, kwargs)))

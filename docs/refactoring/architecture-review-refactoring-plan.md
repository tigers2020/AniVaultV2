...
- **기본값은 직렬(1 worker) 유지**. 병렬은 실험용이며 환경 변수(`ANIVAULT_MATCH_MAX_WORKERS`)로만 활성화한다.
- **근거는 코드 주석**(`match_series._match_max_workers`)에 남기고, **최종 정책·체크리스트 반영은 본 문서**에서 관리한다.
...
- 병렬 유지/축소/제거 중 하나를 명확히 결정(현재: **기본 직렬 유지 + 병렬은 옵트인 실험용**)
- 근거는 코드 구조 분석 또는 측정으로 남김
